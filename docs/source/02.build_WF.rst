...
.. attention:: When executing **NeatSeq-Flow** within a `conda` environment, **NeatSeq-Flow** will add the path to the modules repository automatically (See :ref:`installing with conda`). You don't have to worry about setting it in the parameter file unless you have your own modules installed in a different location.
...
``qsub_params``
    Set cluster related params which will be effective for the current step alone:
...
.. Attention:: As of **NeatSeq-Flow** version 1.1.0, the sample file can only contain **sample files**. No project files are permitted. 
