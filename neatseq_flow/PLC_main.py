...
""" % \
            ...
            logf.write("""
Timestamp\tEvent\tModule\tInstance\tJob name\tHost\tMax mem
""")
  ...
  return "translate(" + d.x + "," + d.y + ")";
    ...
    def create_log_plotter(self):
        ...
        with open(self.pipe_data["logs_dir"] + "log_file_plotter.R", "w") as log_plot:
            log_plot.write(""" 
library(reshape2); library(googleVis); args <- commandArgs(trailingOnly =T);log_file <- read.delim(args[1],skip = 8, he=T,stringsAsFactors = F);parsed_log_file <- dcast(data = log_file, formula = Module+Instance+Job.name~Event, value.var = "Timestamp", fun.aggregate = function(x) x[1]);parsed_log_file$Started <- as.POSIXct(parsed_log_file$Started, tz = "", format="%d/%m/%Y %H:%M:%S ");parsed_log_file$Finished <- as.POSIXct(parsed_log_file$Finished, tz = "", format="%d/%m/%Y %H:%M:%S ");parsed_log_file <- parsed_log_file[not(is.na(parsed_log_file$Finished)|is.na(parsed_log_file$Started)),];parsed_log_file <- parsed_log_file[order(parsed_log_file$Started),];color_leg <- substring(rainbow(length(unique(parsed_log_file$Module))),first = 1,last = 7);names(color_leg) <- unique(parsed_log_file$Module);plotcolors <- sprintf("[%s]",paste(paste("'",color_leg[parsed_log_file$Module],"'",sep=""),collapse = ","));parsed_log_file$Started_numeric <- as.numeric(parsed_log_file$Started);PL_start <- min(parsed_log_file$Started_numeric);parsed_log_file$Started_numeric <- parsed_log_file$Started_numeric - PL_start;parsed_log_file$Finished_numeric <- as.numeric(parsed_log_file$Finished);parsed_log_file$Finished_numeric <- parsed_log_file$Finished_numeric - PL_start;Timeline <- gvisTimeline(data=parsed_log_file, rowlabel="Instance", barlabel="Job.name", start="Started_numeric", end="Finished_numeric", options=list(timeline="{groupByRowLabel:false}", backgroundColor='#ffd', height=800, width=1000, colors=plotcolors));print(x = Timeline, tag=NULL, file = sprintf("%s.html",args[1]));## https://github.com/al2na/Rmarkdown_JSviz/blob/master/googleVis.Rmd
...
if(not(all(c("DiagrammeR","htmlwidgets") %%in%% installed.packages()))) {
